...
class ListTests(TestCase):
    ...
    def test_get_list_properties_should_combine_values(self):
        ...
        b.phones = [1, 2, 5, 789]
        self.list = List(a, b)
        self.assertEqual(self.list.phones.sort(), [1, 2, 123321, 4, 1, 2, 5, 789].sort())
    ...
    def test_non_list_peoperties_with_unique_object_peoperty(self):
        a, b = Mock(), Mock()
        a.foo = 1
        b.bar = 2
        self.list = List(a, b)
    ...
    def test_add_method(self):
        ...
        self.assertTrue(isinstance(added_list, List))

    def test_times_method(self):
        timed_list = self.list * 3
        self.assertTrue(len(timed_list) == 6)
        self.assertTrue(isinstance(timed_list, List))

    def test_slice_method(self):
        sliced_list = self.list[:-1]
        self.assertTrue(len(sliced_list) == 1)
        self.assertTrue(isinstance(sliced_list, List))

    def test_non_existing_attribute_raises_attribute_error(self):
        self.assertRaises(AttributeError, self.list.__getattribute__, 'blabla')
    ...
    def test_first_method_with_nonexisisting_value_raises_indexerror(self):
        self.assertRaises(IndexError, self.list.first, lambda p: p.name == 'id that dont exist')

    def test_first_method_with_nonexisisting_value_returns_none(self):
        self.assertEqual(None, self.list.first(lambda p: p.name == 'danny', None))

    def test_any_method(self):
        self.assertEqual(True, self.list.any(lambda p: p.name == 'bob'))

    def test_any_method_with_all_false(self):
        self.assertEqual(False, self.list.any(lambda p: p.name == 'danny'))

    def test_any_method_without_expression(self):
        self.assertEqual(True, self.list.any())
    ...
    def test_all_method(self):
        self.assertEqual(True, self.list.all(lambda p: p.age == 20))

    def test_all_method_with_false_arguments(self):
        self.assertEqual(False, self.list.all(lambda p: p.age == 19))
    ...
    def test_select_method(self):
        expression = lambda e: e.name
        self.assertEqual(self.list.select(expression), self.list.name)

    def test_skip_method(self):
        self.assertEqual(self.list.skip(1)[0], self.list[1])

    def test_take_method(self):
        self.list = self.list * 6
        taken_elements = self.list.take(2)
        self.assertEqual(taken_elements, self.list[:2])
    ...
    def test_except_for_method(self):
        excepted_list = self.list.except_for(lambda x: x.name.lower().startswith('b'))
        self.assertEqual(len(excepted_list), 1)
        self.assertEqual(excepted_list[0], self.list[1])

    def test_one_liner(self):
        self.assertEqual(20, self.list.concat(self.list).where(lambda e: e.name.lower() == 'john').skip(1).select(
            lambda x: x.age).avg)
        self.list[1].age = 89
